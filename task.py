...
class Task():
    def __init__(self, config, dict):
        ...
        self._org_path = os.path.join(dict.get('additional').get(
            'detail').get('destination'), self.file_name)
        self.type = 'unknown'
        self.title = ''
        ...
        try:
            ...
            if m:
                ...
                if m.group('e1') and m.group('e2'):
                    rename_list = [self.title, '.E', m.group('e1').zfill(
                        2), 'E', m.group('e2').zfill(2), m.group('suf')]
                else:
                    rename_list = [self.title, '.E',
                                   m.group('ep'), m.group('suf')]
                ...
                self.logger.info('rename: ' + self.file_name +
                                 ' -> ' + self.rename_file_name)
            else:
